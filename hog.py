...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0 :
            ...
            score = take_turn(roll0,score0,score1,dice,goal)
            score0 += score
            pile0 = hog_pile(score0,score1)
            score0 += pile0
        else :
            roll1 = strategy1(score1,score0)
            score = take_turn(roll1,score1,score0,dice,goal)
            score1 += score
...
...     return say
>>> s0, s1 = play(always_roll(1), always_roll(1), dice=make_test_dice(5), goal=15, say=count(1))   >>> print(message)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i = 1
    for i in range (1,11) :
       ad = make_averaged(roll_dice,total_samples)
       aver = ad(i,dice) 
       if aver > highS :
            highS = aver 
            trail = i       
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    add = hefty_hogs(score,opponent_score) 
    if add >= threshold :
        return 0
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    add = hefty_hogs(score,opponent_score)
    if hog_pile(score+add,opponent_score) > 0 :
        return 0
